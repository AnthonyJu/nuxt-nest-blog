  ...
  <el-space :size="10" spacer="|">
    ...
    <el-dropdown>
      <Iconify
        ...
        class="h-26px w-40px cursor-pointer text-$el-color-primary"
      />
      <template #dropdown>
        <el-dropdown-menu>
    ...
    <el-dropdown v-if="userStore.userInfo">
      <el-avatar class="cursor-pointer bg-$el-color-primary" :src="userStore.userInfo?.avatar || avatar" />
      <template #dropdown>
        <el-dropdown-menu>
...
function exitLogin() {
  ElMessageBox({
    title: t('layout.exitLoginTip'),
    message: t('layout.exitLoginMessage'),
    showCancelButton: true,
    beforeClose: (action, instance, done) => {
      if (action === 'confirm') {
        instance.confirmButtonLoading = true
        instance.confirmButtonText = t('layout.exitLoginLoading')
